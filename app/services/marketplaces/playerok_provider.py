...
from app.services.marketplaces.playerok_analytics import (
    compute_competition_metrics as compute_playerok_competition_metrics,
    compute_demand_stats as compute_playerok_demand_stats,
    coverage_status as playerok_coverage_status,
    dedupe_reviews as dedupe_playerok_reviews,
    derive_review_tokens_from_offers as derive_playerok_review_tokens_from_offers,
    extract_amounts_from_text as extract_playerok_amounts_from_text,
    filter_offers_by_currency as filter_playerok_offers_by_currency,
    is_amount_close as is_playerok_amount_close,
    is_review_match as is_playerok_review_match,
    is_this_month_bucket as is_playerok_this_month_bucket,
    normalize_currency as normalize_playerok_currency,
    offers_stats as playerok_offers_stats,
    percentile as playerok_percentile,
    review_signature as playerok_review_signature,
)
...
class PlayerOkProvider(MarketplaceProvider):
    ...
    def _percentile(values: list[float], percentile: float) -> float | None:
        return playerok_percentile(values, percentile)

    @classmethod
    def _offers_stats(cls, offers: list[PlayerOkOfferData]) -> OffersStatsV2DTO:
        return playerok_offers_stats(offers)

    @staticmethod
    def _normalize_currency(raw: str | None) -> str | None:
        return normalize_playerok_currency(raw)
    ...
    def _filter_offers_by_currency(
        ...
        return filter_playerok_offers_by_currency(offers, requested_currency)

    @staticmethod
    def _extract_amounts_from_text(value: str) -> list[float]:
        return extract_playerok_amounts_from_text(value)

    @staticmethod
    def _is_amount_close(amount: float, prices: list[float]) -> bool:
        return is_playerok_amount_close(amount, prices)

    @classmethod
    def _derive_review_tokens_from_offers(cls, offers: list[PlayerOkOfferData]) -> list[str]:
        return derive_playerok_review_tokens_from_offers(
            offers,
            token_stopwords=cls.REVIEW_TOKEN_STOPWORDS,
        )

    @staticmethod
    def _is_this_month_bucket(value: str | None) -> bool:
        return is_playerok_this_month_bucket(value)
    ...
    def _compute_demand_stats(
        ...
        *,
        ...
        return compute_playerok_demand_stats(
            relevant_reviews,
    ...
    def _coverage_status(counter_total: int | None, loaded_count: int) -> str:
        return playerok_coverage_status(counter_total, loaded_count)

    @staticmethod
    def _compute_competition_metrics(offers: list[PlayerOkOfferData]) -> dict[str, float | None]:
        return compute_playerok_competition_metrics(offers)
