...
class PlatiMarketProvider(MarketplaceProvider):
    ...
    def _compute_competition_metrics(offers: list[PlatiOfferCard]) -> dict[str, float | None]:
        ...
        prices = [float(item.price) for item in offers if item.price is not None]
        p10 = PlatiMarketProvider._percentile(prices, 0.10)
        p90 = PlatiMarketProvider._percentile(prices, 0.90)
        p50 = PlatiMarketProvider._percentile(prices, 0.50)
        spread = None
    ...
    def _is_recent_30d(review: PlatiReview) -> bool:
        if review.created_at:
            try:
                ...
                pass
        return PlatiMarketProvider._is_this_month_bucket(review.date_bucket)
    ...
    def _compute_demand(
        ...
        *,
        ...
        volume_30d = sum(1 for item in relevant_reviews if cls._is_recent_30d(item))
